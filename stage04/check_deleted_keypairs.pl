...
foreach my $line (@lines){
	...
	if( $line =~ /^ACCOUNTNAME/ ){
		...
		print $line . "\n";
		check_dashboard_element_for_deleted_keypair($line);
	};	
};
...
sub check_dashboard_element_for_deleted_keypair{
	my $line = shift @_;

	if( $line =~ /^\[DASHBOARD\]\s+Keypairs:\s+(\d+)/ ){
		if( $1 != 0 ){
			print "[TEST_RESULT]\tFAILED: TO DELETE KEYPAIR VIA USER CONSOLE !!\n";
		};
	};
	return;
};
