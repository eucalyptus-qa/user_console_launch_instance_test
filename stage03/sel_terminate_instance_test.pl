...
my $this_test = "terminate_instance_basic";
...
for(my $i=0; $i<4; $i++){
	...
	if( $line =~ /^(\S+)\s+(\S+)\s+(\S+)/ ){
		...
		if( $user ne "admin" ){
			...
			$sel_cmd .= " ./runtest_instance.py -i $clc_ip -p $userconsole_port -a $account -u $user -w $password -t $this_test";
