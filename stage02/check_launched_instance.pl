...
foreach my $line (@lines){
	...
	if( $line =~ /^ACCOUNTNAME/ ){
		...
		print $line . "\n";
		check_dashboard_element_for_launched_instance($line);
	};	
};
...
sub check_dashboard_element_for_launched_instance{
	my $line = shift @_;

	if( $line =~ /^\[DASHBOARD\]\s+Running Instances:\s+(\d+)/ ){
		if( $1 != 1 ){
			print "[TEST_RESULT]\tFAILED: TO LAUNCH INSTANCE VIA USER CONSOLE !!\n";
		};
	};
	return;
};
